...
| 字幕形式 | galgame 式逐句：第一句淡入居中 → 停留 → 淡出 → 第二句…… 依次播放**前 3 句**（`MAX_OPENING_LINES = 3`，句数可调） |
| 字幕文本来源 | 世界书 `data/worldbook.json` 的 `first_mes`，按中文标点断句（。！？…；）成句子数组、取前 3 句；世界书缺失/加载失败 → 回退代码内置默认文本前 3 句 |
...
- `loadOpeningLines()`（改造现有 `loadOpeningNarrative()`）：
  ...
  3. 取前 `MAX_OPENING_LINES`（= 3）句返回；空数组 = 加载失败
  4. 失败/空 → 回退内置默认句子数组（现有默认 3 段文本按同样断句规则预切，同样取前 3 句）
